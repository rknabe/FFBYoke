...
PID myPID[2] = {PID(&Input[X_AXIS], &Output[X_AXIS], &Setpoint[X_AXIS],
                    Kp[X_AXIS], Ki[X_AXIS], Kd[X_AXIS], DIRECT),
                PID(&Input[Y_AXIS], &Output[Y_AXIS], &Setpoint[Y_AXIS],
                    Kp[Y_AXIS], Ki[Y_AXIS], Kd[Y_AXIS], DIRECT)};
...
void Set_PID_Turnings()
{
  ...
  for (int ax = 0; ax < 2; ax++)
  {
    ...
    myPID[ax].SetOutputLimits(-CfgManager._Pids[ax].Pid.MaxOutput,
                              CfgManager._Pids[ax].Pid.MaxOutput);
  }
}
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(Buttons[0].pinNumber),
                  Push_Button_01_ISR, CHANGE);
...
void SetEffects()
{
  ...
  // lastX - posX;
  effects[X_AXIS].frictionPositionChange = encoder.axis[X_AXIS].positionChange;
  // lastY - posY;
  effects[Y_AXIS].frictionPositionChange = encoder.axis[Y_AXIS].positionChange;
...
void findCenter(int axis_num)
{
...
#ifdef DEBUG
      sprintf(buff, "[%d]: %ld,%ld", axis_num,
              encoder.axis[axis_num].minValue,
              encoder.axis[axis_num].maxValue);
      Serial.println(buff);
#endif
    }
  }
...
#ifdef DEBUG
  sprintf(buff, "[%d]: %ld - 0 - %ld", axis_num,
  ...
  case X_AXIS:
    ...
    Joystick.setXAxisRange(encoder.axis[X_AXIS].minValue,
                           encoder.axis[X_AXIS].maxValue);
    Joystick.setXAxis(encoder.axis[X_AXIS].currentPosition);
    pwm.setPWM(X_AXIS, 0);
    break;
  case Y_AXIS:
    ...
    Joystick.setYAxisRange(encoder.axis[Y_AXIS].minValue,
                           encoder.axis[Y_AXIS].maxValue);
    Joystick.setYAxis(encoder.axis[Y_AXIS].currentPosition);
    pwm.setPWM(Y_AXIS, 0);
    break;
  default:
...
void gotoPosition(int axis_num, int32_t targetPosition)
{
  ...
  while (encoder.axis[axis_num].currentPosition != targetPosition)
  {
    ...
    {
      char buff[64];
      sprintf(buff, "[%d] P: %ld,T: %ld,F: %d", axis_num,
              encoder.axis[axis_num].currentPosition,
              (int32_t)Setpoint[axis_num], (int)Output[axis_num]);
      Serial.println(buff);
